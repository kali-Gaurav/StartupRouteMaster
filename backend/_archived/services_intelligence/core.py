...
class IntelligenceService:
    ...
    def get_personalized_weights(self, user_id: str) -> Dict[str, float]:
        ...
        Queries the user's segment profile and returns segment-aware weights.
        """
        ...
        SEGMENT_WEIGHTS = {
            UserSegment.NEW.value: {"availability": 0.45, "speed": 0.25, "comfort": 0.20, "safety": 0.10},
            UserSegment.OCCASIONAL.value: {"availability": 0.40, "speed": 0.30, "comfort": 0.20, "safety": 0.10},
            UserSegment.FREQUENT.value: {"availability": 0.30, "speed": 0.40, "comfort": 0.15, "safety": 0.15},
            UserSegment.BUDGET.value: {"availability": 0.50, "speed": 0.20, "comfort": 0.10, "safety": 0.20},
            UserSegment.COMFORT.value: {"availability": 0.25, "speed": 0.20, "comfort": 0.40, "safety": 0.15},
            UserSegment.SAFETY.value: {"availability": 0.20, "speed": 0.15, "comfort": 0.15, "safety": 0.50},
        }
        ...
        try:
            ...
            if profile and profile.segment in SEGMENT_WEIGHTS:
                logger.debug(f"👤 Personalized weights for {user_id}: segment={profile.segment}")
                return SEGMENT_WEIGHTS[profile.segment]

            return default_weights
        except Exception as e:
            logger.warning(f"Failed to fetch personalized weights for {user_id}: {e}")
            return default_weights
