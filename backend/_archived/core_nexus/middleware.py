...
class NexusIOGate:
    ...
    def __init__(self, app: ASGIApp):
        self.app = app
        self._last_pressure_alert_at = 0.0
        self._pressure_alert_cooldown_seconds = float(os.getenv("NEXUS_ALERT_COOLDOWN_SECONDS", "15"))
    ...
    async def __call__(self, scope: Scope, receive: Receive, send: Send) -> None:
        ...
        # We shed load UNLESS it's a critical path (SOS/Health/Stats) or forced continue
        path = scope['path']
        method = scope.get("method", "GET")
        is_critical = "sos" in path or "health" in path or "ping" in path or "stats" in path
        
        # [CORS FIX] Never load-shed OPTIONS requests
        if method == "OPTIONS":
            return await self.app(scope, receive, send)
        ...
        if not is_critical and not force_continue:
            ...
            if mem.percent > max_ram or cpu > max_cpu:
                 now = time.time()
                 if now - self._last_pressure_alert_at >= self._pressure_alert_cooldown_seconds:
                     self._last_pressure_alert_at = now
        ...
        try:
            ...
            from core.resilience.exceptions import handle_engine_crash
