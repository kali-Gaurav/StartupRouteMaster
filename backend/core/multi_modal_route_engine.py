...
class MultiModalRouteEngine:
    ...
    def _time_to_minutes(self, t) -> int:
        ...
        Accepts datetime.time, datetime.datetime, or HH:MM[:SS] string.
        """
        if t is None:
            return 0
        # datetime.time
        ...
        # fallback
        raise TypeError(f"Unsupported time type for _time_to_minutes: {type(t)}")
    ...
    def _find_feasible_trips(self, from_stop_id: int, travel_date: date, after_time: int) -> List[Tuple[int, int, int, float]]:
        ...
        for trip_id, stop_times in self.stop_times_map.items():
            trip_info = self.trips_map.get(trip_id)
            if not trip_info:
                continue
    ...
    def _get_walk_transfers(self, stop_id: int, max_distance_m: int = 800) -> List[Dict]:
        """Generate implicit walking transfers to nearby stops (no explicit Transfer record needed)."""
        results = []
        if stop_id not in self.stops_map:
            return results

        src = self.stops_map[stop_id]
        for other_id, other in self.stops_map.items():
            if other_id == stop_id:
                continue
            # Skip if explicit transfer already exists
            explicit = any(t['to_stop_id'] == other_id for t in self.transfers_map.get(stop_id, []))
            if explicit:
                continue
            # compute distance
            lat1, lon1 = float(src.get('lat', src.get('latitude', 0))), float(src.get('lon', src.get('longitude', 0)))
            lat2, lon2 = float(other.get('lat', other.get('latitude', 0))), float(other.get('lon', other.get('longitude', 0)))
            dist_km = self._haversine_km(lat1, lon1, lat2, lon2)
            dist_m = dist_km * 1000
            if dist_m <= max_distance_m:
                # walking speed ~5 km/h -> time in seconds
                walking_seconds = int((dist_km / 5.0) * 3600)
                walking_seconds = max(30, walking_seconds)
                results.append({'to_stop_id': other_id, 'transfer_type': 0, 'min_transfer_time': walking_seconds})
        return results
    ...
    def search_connecting_journeys(self, journeys: List[Dict], min_layover: int = 30,
        ...
        - Accepts direct transfers (same station) and implicit walk transfers (nearby stops).
        - Validates min-transfer-time using explicit Transfer entries when present or heuristic otherwise.
        ...
        # Helper to find stop_id by name (best-effort exact/case-insensitive)
        def _find_stop_id_by_name(name: str):
            for sid, s in self.stops_map.items():
                if s.get('name', '').lower() == name.lower() or s.get('stop_id', '').lower() == name.lower():
                    return sid
            return None

        # Find connections (allow same-station, explicit transfers, and walkable nearby stops)
        for j1 in journeys:
            for j2 in journeys:
                if j1 is j2:
                    continue

                # Candidate connection if j1 destination == j2 source OR they are walk-nearby stops
                same_station = j1['destination'] == j2['source']
                walk_connect = False
                transfer_explicit = False
                transfer_min_seconds = None

                if not same_station:
                    # try to resolve stop ids and see if they are within walkable distance
                    sid_a = _find_stop_id_by_name(j1['destination'])
                    sid_b = _find_stop_id_by_name(j2['source'])
                    if sid_a and sid_b:
                        # explicit transfer entry
                        for t in self.transfers_map.get(sid_a, []):
                            if t['to_stop_id'] == sid_b:
                                transfer_explicit = True
                                transfer_min_seconds = t.get('min_transfer_time')
                                break

                        # walk connector if within configured walking distance
                        if not transfer_explicit:
                            # use 800m default walking threshold
                            try:
                                lat1 = float(self.stops_map[sid_a].get('lat', self.stops_map[sid_a].get('latitude', 0)))
                                lon1 = float(self.stops_map[sid_a].get('lon', self.stops_map[sid_a].get('longitude', 0)))
                                lat2 = float(self.stops_map[sid_b].get('lat', self.stops_map[sid_b].get('latitude', 0)))
                                lon2 = float(self.stops_map[sid_b].get('lon', self.stops_map[sid_b].get('longitude', 0)))
                                dist_km = self._haversine_km(lat1, lon1, lat2, lon2)
                                if dist_km * 1000 <= getattr(Config, 'WALK_TRANSFER_MAX_METERS', 800):
                                    walk_connect = True
                                    # estimate walking transfer seconds
                                    walking_seconds = int((dist_km / 5.0) * 3600)
                                    transfer_min_seconds = max(30, walking_seconds)
                            except Exception:
                                walk_connect = False

                # if neither same_station nor walk_connect nor explicit transfer, skip
                if not (same_station or walk_connect or transfer_explicit):
                    continue
                ...
                if not j1_arrival or not j2_departure:
                    continue

                layover_minutes = (j2_departure - j1_arrival).total_seconds() / 60

                # Enforce explicit transfer minimums if provided
                min_required_minutes = min_layover
                if transfer_min_seconds is not None:
                    min_required_minutes = max(min_required_minutes, (transfer_min_seconds + 59) // 60)

                if not (min_required_minutes <= layover_minutes <= max_layover):
                    # not a valid connection due to transfer time constraints
                    continue

                combined = self._combine_journeys(j1, j2, layover_minutes)

                # annotate how the transfer is done
                if same_station:
                    combined['transfer_mode'] = 'station'
                elif transfer_explicit:
                    combined['transfer_mode'] = 'timed_transfer'
                elif walk_connect:
                    combined['transfer_mode'] = 'walk'
                else:
                    combined['transfer_mode'] = 'transfer'

                # if the physical stops differ, include both names
                if j1['destination'] != j2['source']:
                    combined['connecting_stations'] = [j1['destination'], j2['source']]

                connecting_options.append(combined)

        # Sort by feasibility score if present, otherwise by duration
        connecting_options.sort(key=lambda x: (-x.get('feasibility_score', 0), x.get('total_duration_minutes', 1)))
    ...
    def simulate_real_time_delays(self, journey: Dict, disruption_db: Session = None) -> Dict:
        ...
        for segment in delayed_journey['segments']:
            ...
            # allow departure_time/arrival_time to be strings or time objects
            dep_time = segment.get('departure_time')
            arr_time = segment.get('arrival_time')
            if isinstance(dep_time, str):
                dep_time = datetime.strptime(dep_time, "%H:%M:%S").time()
            if isinstance(arr_time, str):
                arr_time = datetime.strptime(arr_time, "%H:%M:%S").time()

            segment['adjusted_departure'] = (
                datetime.combine(datetime.fromisoformat(journey['departure_date']).date(), dep_time) +
                timedelta(minutes=current_delay)
            ).time()
            segment['adjusted_arrival'] = (
                datetime.combine(datetime.fromisoformat(journey['departure_date']).date(), arr_time) +
                timedelta(minutes=current_delay + int(segment.get('duration_minutes', 0)))
            ).time()
            current_delay += (total_delay / max(1, len(delayed_journey['segments'])))
