...
class ResourceForecaster:
    """
    Task 4.10: Advanced Resource Prediction using Double Exponential Smoothing (Holt-Winters).
    Captures both level and trend for multi-dimensional metrics (CPU, RAM, Latency).
    """
    def __init__(self, alpha: float = 0.3, beta: float = 0.1):
        self.alpha = alpha  # Level smoothing
        self.beta = beta    # Trend smoothing
        self.metrics = {}   # dict of metric_name -> {'level': float, 'trend': float}

    def update(self, metric_name: str, val: float):
        if metric_name not in self.metrics:
            self.metrics[metric_name] = {'level': val, 'trend': 0.0}
            return
        
        m = self.metrics[metric_name]
        last_level = m['level']
        # 1. Update Level
        m['level'] = (self.alpha * val) + ((1 - self.alpha) * (m['level'] + m['trend']))
        # 2. Update Trend
        m['trend'] = (self.beta * (m['level'] - last_level)) + ((1 - self.beta) * m['trend'])

    def predict_next(self, metric_name: str, intervals: int = 1) -> float:
        """Predicts the value for 'n' intervals ahead for a specific metric."""
        if metric_name not in self.metrics:
            return 0.0
        m = self.metrics[metric_name]
        prediction = m['level'] + (intervals * m['trend'])
        
        # If it's CPU, bound by 100%. Otherwise just keep >= 0
        if metric_name == "cpu":
...
class SystemMonitor:
    ...
    async def update_if_stale(self, force: bool = False):
        ...
        async with self._update_lock:
            ...
            self.forecaster.update("cpu", current_cpu)
            ...
            self.forecaster.update("rss", current_rss)
            ...
            self.forecaster.update("p99_latency", p99)
            ...
            predicted_cpu = self.forecaster.predict_next("cpu")
            if predicted_cpu > 95 and self._state < SystemState.WARNING:
                logger.warning(f"🚨 CPU SPIKE PREDICTED: Next: {predicted_cpu:.1f}%")
                
            predicted_rss = self.forecaster.predict_next("rss")
            if predicted_rss > (mem.vms * 0.9) and self._state < SystemState.WARNING:
                logger.warning(f"🚨 MEMORY SPIKE PREDICTED: Next: {predicted_rss/1024/1024:.1f}MB")
    ...
    def stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "resource": {
                ...
                "cpu_predicted": self.forecaster.predict_next("cpu"),
                "ram_percent": self._ram_percent,
                "rss_bytes": self.last_rss,
                "rss_predicted": self.forecaster.predict_next("rss"),
            ...
            "performance": {
                ...
                "p99_ms": p99,
                "p99_latency_predicted": self.forecaster.predict_next("p99_latency"),
