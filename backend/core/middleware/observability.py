...
class ObservabilityMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        ...
        # Suggestion #5: Endpoint Heatmap (fire-and-forget to avoid blocking requests)
        path = request.url.path
        try:
            asyncio.create_task(self._record_request_metrics(path, duration))
        except Exception as e:
            logger.error(f"Metrics task scheduling failed: {e}")

        response.headers["X-Correlation-ID"] = correlation_id
        return response

    async def _record_request_metrics(self, path: str, duration: float):
        """Write latency/heatmap metrics in the background.

        This is intentionally fire-and-forget to ensure user-facing requests
        are not blocked by Redis latency or network issues.
        """
        try:
            ...
            if recent:
                ...
                if p99_sample > 2000:  # 2 seconds
                    ...
                    logger.critical(
                        f"⚠️ AUTO-THROTTLE: p99 sample {p99_sample}ms > 2000ms. Low-latency mode enabled for 60s."
                    )
        except Exception as e:
