...
class FastPathRouter(BaseRoutingEngine):
    ...
    async def find_routes(self, request: RoutingRequest) -> RoutingResponse:
        self.graph = request.graph or self.graph
        if not self.graph: 
            from .engine import route_engine
            self.graph = await route_engine._get_current_graph(request.departure_date)
        return await asyncio.to_thread(self._find_routes_sync, request)

    def _find_routes_sync(self, request: RoutingRequest) -> RoutingResponse:
             ...
             if src_hubs and dst_hubs:
                  for s_hub in src_hubs:
                       for d_hub in dst_hubs:
                            ...
                            all_found.extend(self._find_2_hub_transfers(s_hub, d_hub, window_start, window_end))
    ...
    def _find_2_hub_transfers(self, s_hub: int, d_hub: int, start: datetime, end: datetime) -> List[Route]:
        ...
        deps = self.graph.get_departures_from_stop(s_hub, start, 1440)
        for dep_time, tid in deps:
             # Get all stops for this trip
             stops = self.graph.get_trip_segments(tid)
             for s in stops:
                  mid_hub = s.arrival_stop_id
                  mid_stop = self.graph.stop_cache.get(mid_hub)
                  if mid_stop and mid_stop.code in MEGA_HUBS and mid_hub != s_hub:
                       # Check if mid_hub connects to d_hub
                       connection_deps = self.graph.get_departures_from_stop(mid_hub, s.arrival_time + timedelta(minutes=30), 1440)
                       for c_dep_time, c_tid in connection_deps:
                            if self.graph.can_reach_destination(c_tid, d_hub):
                                 # Found a 2-train hub transfer!
                                 segs1 = self._get_segments(tid, s_hub, mid_hub)
                                 segs2 = self._get_segments(c_tid, mid_hub, d_hub)
                                 if segs1 and segs2:
