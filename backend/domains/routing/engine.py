...
# Imports from core route engine components (keep as-is, these are stable)
# Using relative imports for domains/ tier access
import sys
import os
...
try:
    ...
    # Fallback for when running with backend prefix
...
try:
    ...
    from core.route_engine.data_structures import Route, UserContext
    from core.route_engine.constraints import RouteConstraints
    from core.route_engine.raptor import OptimizedRAPTOR, HybridRAPTOR
    from core.route_engine.graph import (
        TimeDependentGraph,
        StaticGraphSnapshot,
        RealtimeOverlay,
    )
    ...
    from core.ml_integration import get_model_registry
except ImportError:
    ...
    from backend.core.route_engine.data_structures import Route, UserContext
    from backend.core.route_engine.constraints import RouteConstraints
    from backend.core.route_engine.raptor import OptimizedRAPTOR, HybridRAPTOR
    from backend.core.route_engine.graph import (
        TimeDependentGraph,
        StaticGraphSnapshot,
        RealtimeOverlay,
    )
    from backend.core.route_engine.builder import GraphBuilder
    from backend.core.route_engine.hub import HubManager, HubConnectivityTable
    ...
    from backend.core.ml_integration import get_model_registry
...
class RailwayRouteEngine:
    ...
    PHASE 1 CONSOLIDATION: This is the authoritative routing implementation.
    All other route engines (advanced_route_engine.py, etc.) are archived.
    """
    ...
    def _log_startup_status(self):
        ...
        """
        logger.info("=" * 60)
        logger.info("🚀 Railway Route Engine - Phase 1 Consolidation")
        logger.info("📍 Location: domains/routing/engine.py")
        logger.info("=" * 60)

        mode = "OFFLINE"
        if (
            self.data_provider.has_live_fares
            or self.data_provider.has_live_delays
            or self.data_provider.has_live_seats
        ):
            mode = (
                "ONLINE"
                if (
                    self.data_provider.has_live_fares
                    and self.data_provider.has_live_delays
                    and self.data_provider.has_live_seats
                )
                else "HYBRID"
            )
        ...
        logger.info(
            f"   • Fares: {'🌐 LIVE API' if self.data_provider.has_live_fares else '💾 DATABASE'}"
        )
        logger.info(
            f"   • Delays: {'🌐 LIVE API' if self.data_provider.has_live_delays else '⏱️  ASSUME 0'}"
        )
        logger.info(
            f"   • Seats: {'🌐 LIVE API' if self.data_provider.has_live_seats else '💾 DATABASE'}"
        )
    ...
    async def _get_current_graph(self, date: datetime) -> TimeDependentGraph:
        ...
        needs_rebuild = (
            not self.current_snapshot
            or self.current_snapshot.date.date() != date.date()
            or (self.last_snapshot_time is None)  # Rebuild if never built
            or (datetime.utcnow() - self.last_snapshot_time).total_seconds() > 86400  # 24 hours
        )
    ...
    async def _apply_ml_ranking(self, routes: List[Route], user_context: UserContext) -> List[Route]:
        ...
        if self.route_ranking_model.loaded:
            ...
            ranked_routes = await self.route_ranking_model.predict(
                routes, user_context, constraints
            )
    ...
    async def apply_realtime_updates(self, updates: List[Dict[str, Any]]):
        ...
        for update in updates:
            ...
            if update_type == "cancellation":
                ...
                logger.debug(
                    f"Occupancy update for trip {trip_id} received, but not directly applied to graph overlay."
                )
            else:
