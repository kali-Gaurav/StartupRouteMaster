...
try:
    ...
    # Safe dummy cache decorator fallback
    def cache(expire=None, *args, **kwargs):
        def decorator(func):
            import functools
...
from core.infrastructure.metrics import jit_metrics, SurgeLevel
...
async def search_routes_endpoint(
    ...
    try:
        ...
        demand_service.log_search_intent(search_request.source, search_request.destination, search_request.persona or "generic")
...
async def quick_search_endpoint(
    ...
    try:
        ...
        # Log demand intent (Pillar 3)
        demand_service.log_search_intent(source, destination, "generic")
