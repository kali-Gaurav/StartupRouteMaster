...
async def _send_search_results(chat_id: int, source: str, destination: str, travel_date: str, db_session: Session):
    ...
    for idx, journey in enumerate(journeys[:3], start=1):
        journey_id = journey.get("journey_id") or journey.get("train_number") or str(idx)
        ...
        buttons.append([
            {"text": f"Book Option {idx}", "callback_data": f"select_train:{journey_id}"}
        ])

    lines.append("\nChoose one of the options above, or send another route.")
    reply_markup = {"inline_keyboard": buttons + [[{"text": "🔙 Main Menu", "callback_data": "help"}]]}
    return await telegram_dispatcher.send_message(
        chat_id,
        "\n".join(lines),
...
async def start_command_handler(chat_id: int, args: str, db_session: Session):
    ...
    if args:
        token_to_check = args.upper().strip()
        user = db_session.query(User).filter(
            User.telegram_link_token == token_to_check,
            User.telegram_link_expiry > datetime.utcnow()
        ).first()
        if user:
            existing_account = db_session.query(TelegramAccount).filter(
                TelegramAccount.telegram_id == str(chat_id)
            ).first()
            if existing_account and existing_account.user_id != user.id:
                return await telegram_dispatcher.send_message(
                    chat_id,
                    "❌ This Telegram profile is already linked to a different RouteMaster account.",
                    reply_markup=telegram_dispatcher.get_keyboard("help")
                )

            if not existing_account:
                existing_account = TelegramAccount(
                    user_id=user.id,
...
async def help_command_handler(chat_id: int, args: str, db_session: Session):
    ...
    help_message = (
        ...
        "/book - Start a booking flow for a route.\n"
        "/ticket - Get ticket status by PNR.\n"
...
async def book_command_handler(chat_id: int, args: str, db_session: Session):
    """Handles the /book command to start booking search flows."""
    if not args:
        return await telegram_dispatcher.send_message(
            chat_id,
            "Please tell me your journey like 'Mumbai to Delhi on 2026-05-01' so I can find the best booking options.",
            reply_markup=telegram_dispatcher.get_keyboard("default")
        )
    ...
    if intent_res and intent_res.get("intent") == "search" and intent_res.get("entities"):
        ...
        if source and destination:
            session_manager.update_session(
                ...
                intent="booking",
                step="searching",
                context={"source": source, "destination": destination, "travel_date": travel_date}
            )
            return await _send_search_results(chat_id, source, destination, travel_date, db_session)

    return await telegram_dispatcher.send_message(
        chat_id,
        "I couldn't understand your booking request. Please try again like 'Mumbai to Delhi tomorrow'.",
        reply_markup=telegram_dispatcher.get_keyboard("default")
    )


async def ticket_command_handler(chat_id: int, args: str, db_session: Session):
    """Handles the /ticket command as a PNR lookup alias."""
    return await pnr_command_handler(chat_id, args, db_session)
...
command_handler.register_command("/book", async_handler_wrapper(book_command_handler))
command_handler.register_command("/ticket", async_handler_wrapper(ticket_command_handler))
