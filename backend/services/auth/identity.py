...
class IdentityService:
    ...
    def _build_transient_fingerprint(
        self,
        *,
        ...
        """Return an in-memory fingerprint object when persistence must fail open."""
    ...
    async def get_or_create_fingerprint(self, request: Request, user_id: Optional[str] = None) -> IdentityFingerprint:
        ...
        transient_fingerprint = self._build_transient_fingerprint(
            fingerprint_hash=fingerprint_hash,
            ip_address=ip_address,
            user_agent=user_agent,
            user_id=user_id,
        ...
        try:
            fingerprint = (
                self.db.query(IdentityFingerprint)
                .options(
                    load_only(
                        IdentityFingerprint.id,
                        IdentityFingerprint.user_id,
                        IdentityFingerprint.ip_address,
                        IdentityFingerprint.user_agent,
                        IdentityFingerprint.fingerprint_hash,
                        IdentityFingerprint.is_trusted,
                        IdentityFingerprint.risk_score,
                        IdentityFingerprint.created_at,
                    )
                )
                ...
                self.db.execute(
                    text(
                        ...
                        INSERT INTO identity_fingerprints
                            (id, user_id, ip_address, user_agent, fingerprint_hash, is_trusted, risk_score, created_at)
                        VALUES
                            (:id, :user_id, :ip_address, :user_agent, :fingerprint_hash, :is_trusted, :risk_score, :created_at)
                        """
                    ),
                    {
                        "id": transient_fingerprint.id,
                        "user_id": user_id,
                        "ip_address": ip_address,
                        "user_agent": user_agent,
                        "fingerprint_hash": fingerprint_hash,
                        "is_trusted": True,
                        "risk_score": behavioral_risk,
                        "created_at": transient_fingerprint.created_at,
                    },
                )
                self.db.commit()
                logger.info(f"🆕 [SENTINEL S1.5] New identity registered: {fingerprint_hash[:10]}... (Initial Risk: {behavioral_risk})")
                return transient_fingerprint

            fingerprint.risk_score = max(0.0, (float(fingerprint.risk_score or 0.0) + behavioral_risk) / 2)
            ...
            self.db.commit()
            return self._build_transient_fingerprint(
                fingerprint_hash=fingerprint_hash,
                ip_address=ip_address,
                user_agent=user_agent,
                user_id=fingerprint.user_id,
                risk_score=float(fingerprint.risk_score or 0.0),
            )
        except SQLAlchemyError as exc:
            logger.warning(
                "⚠️ [SENTINEL] Identity fingerprint persistence degraded; failing open for search continuity: %s",
                exc,
            )
            try:
                self.db.rollback()
    ...
    def is_trusted_device(self, fingerprint_hash: str) -> bool:
        ...
        try:
            ...
            logger.warning("Identity trust check degraded for %s: %s", fingerprint_hash[:10], exc)
            try:
                self.db.rollback()
            except Exception:
                pass
