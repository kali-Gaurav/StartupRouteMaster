...
class SubscriptionService:
    ...
    async def upgrade_user(
        ...
        subscription.is_pro = tier in ["PRO", "ELITE"]
        subscription.features = PLAN_FEATURES.get(tier, PLAN_FEATURES["FREE"])
        
        # Expiry Logic
        if subscription.expires_at and subscription.expires_at > now:
            subscription.expires_at += timedelta(days=30 * months)
        else:
        ...
        grace_deadline = expires_at + timedelta(days=GRACE_PERIOD_DAYS) if expires_at is not None else None
        ...
        details = SubscriptionDetails(
            ...
            status=self._get_subscription_status(subscription),
            expires_at=expires_at,
            features=subscription.features or PLAN_FEATURES.get(tier, PLAN_FEATURES["FREE"]),
        ...
        logger.info(
            f"✅ User {user_id} upgraded to {tier} (expires: {expires_at.isoformat() if expires_at else 'never'})"
        )
    ...
    async def get_user_subscription(
        ...
        details = SubscriptionDetails(
            ...
            expires_at=expires_at,
            features=sub.features or PLAN_FEATURES.get(sub.plan_tier, PLAN_FEATURES["FREE"]),
            grace_deadline=grace_deadline
        )
