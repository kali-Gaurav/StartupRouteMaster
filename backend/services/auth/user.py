...
class UserService:
    ...
    def get_user_by_firebase_uid(self, firebase_uid: str) -> Optional[User]:
        """
        Cached user lookup by Firebase UID.
        """
        cache_key = self._get_cache_key("firebase_uid", firebase_uid)
    ...
    def create_user_with_data(self, data: dict) -> User:
        ...
        for Firebase/OTP flows where we may only have phone or email.
        """
        firebase_uid = data.get("firebase_uid") or data.get("supabase_id")
        ...
        # Create profile if firebase_uid provided
        if firebase_uid:
            existing_profile = self.db.query(Profile).filter(Profile.id == firebase_uid).first()
        ...
        if firebase_uid:
            self._invalidate_firebase_cache(firebase_uid)
    ...
    def _invalidate_firebase_cache(self, firebase_uid: str) -> None:
        """Invalidate cache for a Firebase UID."""
        cache_key = self._get_cache_key("firebase_uid", firebase_uid)
        self._cache.pop(cache_key, None)
    ...
    def update_user_profile(self, user: User, changes: dict) -> User:
        ...
        if user.firebase_uid:
            self._invalidate_firebase_cache(user.firebase_uid)
