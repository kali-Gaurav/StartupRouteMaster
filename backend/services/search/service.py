...
from core.infrastructure.metrics import jit_metrics, SurgeLevel, DegradationManager
...
class SearchService:
    ...
    def __init__(self, db: Optional[Session] = None, route_engine_instance: Optional[RailwayRouteEngine] = None):
        ...
        # Defensive init: UnlockService may fail if circuit breaker infra isn't ready
        try:
            self.unlock_service = UnlockService()
        except Exception as e:
            logger.warning(f"UnlockService init failed, using lazy fallback: {e}")
            self._unlock_service_lazy = None
        ...
        self._transit_db_breaker = circuit_manager.get_or_create(
            "search_transit_db",
        ...
        from core.resilience.core import TokenBucketRateLimiter
        # [RM-003] Backpressure Limiter for SSE (Tokens/sec)
        self.stream_limiter = TokenBucketRateLimiter(rate=50.0, capacity=100.0)

    async def _safe_call(self, func: Callable, *args, fallback: Any = None, **kwargs) -> Any:
        """Generic wrapper for resilient internal service calls."""
        try:
            if asyncio.iscoroutinefunction(func):
                return await func(*args, **kwargs)
            return func(*args, **kwargs)
        except Exception as e:
            logger.warning(f"Resilience: Safe call to {func.__name__ if hasattr(func, '__name__') else 'unknown'} failed: {e}")
            return fallback

    def _safe_mask_route(self, route_dict: Dict[str, Any]) -> Dict[str, Any]:
        """Safely mask a route dict, with fallback if unlock_service is unavailable."""
        try:
            # Check if unlock_service was initialized
            if hasattr(self, 'unlock_service') and self.unlock_service:
                return self.unlock_service.mask_route(route_dict, self.unlock_service.config.unlock_fee)
            return UnlockService.mask_route(route_dict)
        except (AttributeError, Exception):
            # Fallback to static method if instance method fails or service is missing
            return UnlockService.mask_route(route_dict)
    ...
    from services.analytics.telemetry import telemetry_collector
    ...
    async def search_routes(
        ...
        # NOTE: gc.disable() removed — disabling GC globally in a concurrent web server
        # causes unbounded memory growth across all in-flight requests.
        ...
        try:
            ...
            if len(all_unique_routes) == 0 and budget > 0.3:
                ...
                # [Task 6] Collect engine diagnostics for zero-yield analysis
                engine_diags = {}
                hub_raw_results = await asyncio.gather(*hub_tasks, return_exceptions=True)
                for res_list in hub_raw_results:
                    try:
                        if not res_list or isinstance(res_list, Exception):
                            # Search for engine name in task wrapper metadata if possible
                            pass 
                    except Exception: pass

                if len(all_unique_routes) == 0:
                    logger.warning(f"🚨 [ORCHESTRATOR] Zero yield for {source}->{destination}. Engines failed to find valid paths.")
                    ...
                    for l1 in leg1_list:
                        for l2 in leg2_list:
                            ...
                            if min_buffer <= wait <= 720:
                                # Create joined route
                                from core.data_utils.structures import Route, TransferConnection
                                merged = Route()
            ...
            if quota == "GN" and high_risk_count >= (len(verified_routes) / 2) and len(verified_routes) > 0:
                ...
                from services.pricing import PricingOrchestrator as PricingService
                ...
                if verified_routes:
                    # [RM-002] Parallelize metadata injection to avoid N+1 latency
                ...
                if multi_layer_cache.redis and verified_routes:
                    await multi_layer_cache.put(discovery_key, verified_routes, ttl=TTL_ROUTE_SEARCH, use_pickle=True)
            ...
            def final_rank_score(r):
                ...
                persona_value = persona.value.upper() if isinstance(persona, Persona) else str(persona).upper()
                if persona_value in ["ECONOMY", "FAMILY"]:
            ...
            masked_journeys = [self._safe_mask_route(rd) for rd in all_hydrated_dicts]
            ...
            from core.data_utils.structures import PaginationMetadata
            ...
            final_response = {
                ...
                "data": {
                    ...
                    "grouped_journeys": {k: ([self._safe_mask_route(r) for r in v] if isinstance(v, list) else v) for k, v in categories.items()},
    ...
    async def stream_routes(
        ...
        # [Day 10] True Async Streaming for TTFR optimization
        async for chunk in orchestrator.stream_all_tiers(req):
            if request and await request.is_disconnected():
                logger.warning("🚫 Stream Aborted: Client disconnected.")
                break

            # Apply Backpressure [RM-003]
            # Ensure we don't overwhelm the client buffer/memory
            await self.stream_limiter.wait_for_token(len(chunk))

            # Mask and Hydrate
            masked_chunk = [self._safe_mask_route(r.to_dict()) for r in chunk]
            final_verified.extend(chunk)
        ...
        # 3. Finalize Stream (Sovereign Context)
        try:
            from core.sovereign.orchestrator import sio
            sio_res = await sio.execute_search_cycle(
                source=source,
                destination=destination,
                initial_routes=final_verified,
            ...
            logger.error(f"Sovereign Stream Finalization failed: {sio_err}")
            yield b"event: end\ndata: {}\n\n"
        finally:
            # Ensure transit session is always closed to prevent connection pool exhaustion
            try:
                transit_db.close()
            except Exception:
                pass
    ...
    async def load_more_routes(self, session_id: str, limit: int = 15, quota: str = "GN", cursor: Optional[float] = None, category: Optional[str] = None) -> Dict[Any, Any]:
        ...
        masked = [self._safe_mask_route(r.to_dict()) for r in verified]
    ...
    async def re_rank_routes(self, session_id: str, persona_str: str, quota: str = "GN") -> Dict[Any, Any]:
        ...
        return {
            ...
            "persona": persona_str,
            "data": {
                "journeys": [self._safe_mask_route(r.to_dict()) for r in sorted_routes[:15]],
                "grouped_journeys": {k: [self._safe_mask_route(r) for r in v] for k, v in categories.items()}
            },
            "metadata": {"latency_ms": (time.time() - start_time) * 1000}
        }
    ...
    async def search_routes_stream(
        ...
        for batch in routes_batches:
            ...
            speculative_masked = [self._safe_mask_route(r.to_dict()) for r in batch]
            ...
            masked_chunk = [self._safe_mask_route(r.to_dict()) for r in verified_chunk if r.metadata.get("is_verified")]
