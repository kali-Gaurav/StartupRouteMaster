...
class AnalyticsConsumer:
    ...
    async def process_event(self, event_data: Optional[Dict[str, Any]], topic: str):
        """Process individual event"""
        self.last_message_time = datetime.utcnow()
        self.processed_events += 1

        if not isinstance(event_data, dict):
            logger.warning(f"Skipping invalid analytics event payload: {event_data!r}")
            return
    ...
    async def _process_route_searched(self, event: Dict[str, Any]):
        ...
        routes_shown = int(event.get('routes_shown', 0) or 0)
        latency = float(event.get('search_latency_ms', 0) or 0.0)
            ...
            if now.hour == 3 and now.minute == 0:
                if os.path.exists(filepath):
                    try:
                        ...
                        logger.info(f"🧹 [NEXUS:VACUUM] Replaying {len(lines)} ghost events!")
                        for line in lines:
                            try:
                                payload = json.loads(line)
                                await self.process_event(payload.get("data"), "fallback_replay")
                            except Exception as e:
                                logger.error(f"[NEXUS:VACUUM] Failed to replay event: {e}")
                                
                        os.remove(filepath)
                        logger.info(f"🧹 [NEXUS:VACUUM] Successfully vacuumed and replayed {len(lines)} ghost events!")
                    except Exception as e:
