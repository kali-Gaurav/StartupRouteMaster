...
class UnlockService:
    ...
    def __init__(self, config: Optional[UnlockServiceConfig] = None):
        ...
        self._breaker = circuit_manager.get_or_create("unlock_service")
    ...
    @staticmethod
    def mask_route(route_dict: Dict[str, Any], unlock_fee: float = 49.0) -> Dict[str, Any]:
        """
        ...
        masked["unlock_fee"] = unlock_fee
        masked["unlock_currency"] = "INR"
        masked["cta_text"] = f"Unlock for ₹{unlock_fee:,.0f}"
    ...
    @circuit_manager.get_or_create("unlock_service").decorate
    @retry_sync
    def create_unlock_request(
        ...
        try:
            ...
            return UnlockRequestResult(
                success=True,
                booking_id=str(unlock_booking.id),
                route_id=route_id,
                user_id=user_id,
                amount=unlock_amount,
    ...
    def fulfill_unlock(
        ...
        try:
            ...
            # Invalidate cache
            self._invalidate_cache(str(booking.user_id), str(booking.route_id))
            
            # Record metrics
            self._record_metric("fulfill_unlock", True, str(booking.route_id))
...
def apply_route_locks(
    ...
    for index, route in enumerate(routes):
        ...
        if index < preview_count:
            ...
            route_payload = unlock_service.mask_route(route_payload)
