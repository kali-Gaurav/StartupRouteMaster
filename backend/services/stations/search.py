...
try:
    ...
    class FallbackProcess:
        @staticmethod
        def extract(query: str, choices: Any, limit: int = 5, scorer: Any = None) -> List[Tuple[Any, float, int]]:
            results = []
            scorer_func = scorer.ratio if hasattr(scorer, 'ratio') else FallbackFuzz.ratio
