...
class ConversationEngine:
    ...
    async def handle_message(self, chat_id: int, user_id_or_text: str | None = None, text: str | None = None):
        if text is None:
            text = user_id_or_text or ""
            telegram_user_id = str(chat_id)
        else:
            telegram_user_id = str(user_id_or_text) if user_id_or_text is not None else str(chat_id)

        user_id = telegram_user_id
        session = session_manager.get_or_create_session(self.db, str(chat_id), telegram_user_id)
    ...
    async def _handle_booking_flow(self, chat_id: int, user_id: str, text: str, session):
        ...
        if session.current_step == "confirm_pax":
            return await telegram_dispatcher.send_message(
                chat_id,
                "✅ I have your passenger details. Tap 'Proceed to Payment' or add more passengers.",
                reply_markup={
                    "inline_keyboard": [
                        [{"text": "➕ Add Another", "callback_data": "add_pax"}],
                        [{"text": "💳 Proceed to Payment", "callback_data": "select_payment_method"}]
        ...
        if session.current_step == "payment_initiated":
            return await telegram_dispatcher.send_message(
                chat_id,
                "⏳ Your payment is pending confirmation. Please complete the payment and then tap 'I have paid'.",
                reply_markup={
                    "inline_keyboard": [
        ...
        return await telegram_dispatcher.send_message(
            chat_id,
            "I’m still processing your booking. Please select a journey from the search results or type /book to start again.",
    ...
    async def handle_callback(
        ...
        chat_id: int,
        user_id_or_data: str,
        data_or_callback_id: str | None = None,
        callback_query_id: str | None = None
    ):
        """Processes inline keyboard callbacks."""
        if data_or_callback_id is None:
            data = user_id_or_data
            callback_query_id = None
            user_id = str(chat_id)
        elif callback_query_id is None:
            data = user_id_or_data
            callback_query_id = data_or_callback_id
            user_id = str(chat_id)
        else:
            user_id = user_id_or_data
            data = data_or_callback_id

        session = session_manager.get_or_create_session(self.db, str(chat_id), user_id)
        
        # Acknowledge callback if we have callback metadata
        ...
        if action == "book_trip":
            return await telegram_dispatcher.send_message(
                chat_id,
                "To book a trip, tell me your journey in this format: Delhi to Mumbai tomorrow.",
                reply_markup=telegram_dispatcher.get_keyboard("default")
            )
        ...
        if action == "ticket":
            return await telegram_dispatcher.send_message(
                chat_id,
                "Please provide your 10-digit PNR number, or use /ticket <PNR>.",
                reply_markup=telegram_dispatcher.get_keyboard("default")
            )
