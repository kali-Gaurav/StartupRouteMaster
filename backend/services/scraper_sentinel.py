...
class ScraperSentinel:
    ...
    def __init__(self, recycling_threshold: int = 50):
        ...
        # [Task 48.3] Scaling Thresholds
        self.BASE_MAX_CONTEXTS = 3
        self.BURST_MAX_CONTEXTS = 10
    ...
    def record_failure(self, source: str):
        """[Task 48.7] Track failures and trip breaker."""
        self._failure_counts[source] = self._failure_counts.get(source, 0) + 1
        if self._failure_counts[source] >= 5:
            logger.error(f"🚨 Circuit Breaker Tripped for {source}. Disabling for 10m.")
            self._disabled_sources[source] = time.time() + 600

    async def start(self):
        """Initialize browser and warm pool."""
        async with self._lock:
            if not self._pw:
                logger.info("🎬 Starting Scraper Sentinel (Playwright)...")
    ...
    async def _create_new_context(self) -> Dict[str, Any]:
        ...
        # [Task 48.2] Anti-Detection Scripts
        # await context.add_init_script(...) # WebGL/Signature Masking
    ...
    async def acquire_context(self) -> Dict[str, Any]:
        ...
        """
        async with self._lock:
            # 1. Clean up stale/leaking contexts first
    ...
    async def release_context(self, entry: Dict[str, Any]):
        """Returns context to pool or marks for recycling."""
        entry["in_use"] = False
        entry["usage_count"] += 1
        entry["last_used_at"] = time.time()
        
        # [Task 48.6] Garbage Collection
        if entry["usage_count"] >= self.recycling_threshold:
            logger.info("♻️ Recycling high-usage context.")
    ...
    async def _reap_zombies(self):
        ...
        for entry in list(self._contexts):
            # Context active for > 5 mins without release
            if entry["in_use"] and (now - entry["last_used_at"] > 300):
                logger.warning("🧟 Reaping zombie context.")
