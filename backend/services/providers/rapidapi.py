...
from core.resilience.core import CircuitState, CircuitBreaker
...
class RapidApiProvider(ServiceProvider):
    def __init__(self):
        ...
        from core.resilience.core import CircuitConfig
        ...
        from core.resilience.retry import RETRY_POLICY_EXTERNAL_API
    ...
    async def _make_request(self, method: str, endpoint: str, params: Optional[Dict] = None, ttl: int = 300) -> Optional[Dict]:
        """
        Makes a request to the RapidAPI endpoint with caching, error handling, and circuit breaking.
        [VYA Optimization] Supports custom TTL per endpoint to conserve quota.
        """
        if (self.http_session is None or self.http_session.closed) and Config.RAPIDAPI_KEY:
            async with self._init_lock:
                if self.http_session is None or self.http_session.closed:
                    await self.init()

        if not self.is_healthy:
            logger.warning("RapidAPI Provider is not healthy or session is not initialized.")
            return None
        ...
        # Caching layer
        # [Production Fix] Use json.dumps for safe, deterministic cache keys
        # frozenset(params.items()) crashes on unhashable values (lists, dicts, nested structures)
        import json as _json
        param_hash = _json.dumps(params, sort_keys=True, default=str) if params else ''
        cache_key = f"rapidapi:{endpoint}:{param_hash}"
        ...
        try:
            ...
            data = await self.breaker.execute_async(lambda: self._execute_request(method, full_url, params))
            if data:
                # Cache the successful response with custom TTL
                await multi_layer_cache.put(cache_key, data, ttl=ttl)
            return data
        except Exception as e:
    ...
    async def get_fare_v2(self, train_no: str, from_station_code: str, to_station_code: str) -> Optional[FareV2]:
        """Fetches fare from /api/v2/getFare (TTL: 24h)"""
        endpoint = "/api/v2/getFare"
        params = {"trainNo": train_no, "fromStationCode": from_station_code, "toStationCode": to_station_code}
        data = await self._make_request("GET", endpoint, params, ttl=86400)
        if data and data.get("status"):
            try:
    ...
    async def get_live_train_status(self, train_no: str, start_day: int = 0) -> Optional[LiveTrainStatus]:
        """Fetches live train status from /api/v1/liveTrainStatus (TTL: 5m)"""
        endpoint = "/api/v1/liveTrainStatus"
        params = {"trainNo": train_no, "startDay": start_day}
        data = await self._make_request("GET", endpoint, params, ttl=300)
        if data and data.get("status"):
            try:
    ...
    async def get_train_schedule(self, train_no: str) -> Optional[TrainSchedule]:
        """Fetches train schedule from /api/v1/getTrainSchedule (TTL: 7 days)"""
        endpoint = "/api/v1/getTrainSchedule"
        params = {"trainNo": train_no}
        data = await self._make_request("GET", endpoint, params, ttl=604800) # 7 days
        if data and data.get("status"):
            try:
    ...
    async def check_seat_availability(self, train_no: str, from_station: str, to_station: str, date: str, class_type: str, quota: str) -> Optional[SeatAvailability]:
        """Checks seat availability from /api/v2/checkSeatAvailability (TTL: 30 mins)"""
        ...
        data = await self._make_request("GET", endpoint, params, ttl=1800) # 30 mins
        if data and data.get("status"):
            try:
