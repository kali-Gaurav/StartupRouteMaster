...
class AegisForgeService:
    ...
    _MAX_CONCURRENT_DRILLS = 3
    _active_drills: Dict[str, DisasterDrill] = {}
    _drills_lock = asyncio.Lock()
    ...
    def __init__(self):
        ...
        # Drill history
        self._drill_history: deque = deque(maxlen=100)
        ...
        try:
            ...
            if db:
                try:
                    ...
                    await intel_svc.log_safety_event(
                        "CHAOS_DRILL_START",
                        f"Scenario: {drill_name}",
                        severity="WARNING"
                    )
            ...
            async def run_storm():
                ...
                try:
                    async with SessionTransit() as db:
                        ...
                        for _ in range(100):
                            tasks.append(
                                search_svc.route_engine.find_routes_direct(
                                    "NDLS", "AGC", datetime.now()
                                )
                            )
    ...
    async def _run_circuit_breaker_test(cls) -> Dict[str, Any]:
        ...
        return {
            ...
            "results": results,
            "open_circuits": len(circuit_breaker_manager.get_open_circuits())
        }
    ...
    async def run_resilience_audit(db) -> Dict[str, Any]:
        ...
        circuit_health = circuit_breaker_manager.get_all_health()
        open_circuits = len(circuit_breaker_manager.get_open_circuits())
