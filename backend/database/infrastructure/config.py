...
class Config(metaclass=ConfigMeta):
    ...
    SLIM_MODE = os.getenv("SLIM_MODE", "false").lower() in ("1", "true", "yes")
    ENVIRONMENT = os.getenv("ENVIRONMENT", "production").lower()
    PAYMENT_RECONCILIATION_INTERVAL_MINUTES = int(os.getenv("PAYMENT_RECONCILIATION_INTERVAL_MINUTES", "15"))
    INVENTORY_RECONCILIATION_INTERVAL_SECONDS = int(os.getenv("INVENTORY_RECONCILIATION_INTERVAL_SECONDS", "30"))
    ...
    DB_DIR = os.path.join(DATA_DIR, "storage") # Dedicated storage folder
    ...
    def GET_SQLALCHEMY_URL(cls, db_type: str = "user", is_async: bool = True) -> str:
        ...
        def resolve_db_path(filename):
            path_str = str(Path(cls.DB_DIR) / filename).replace('\\', '/')
            # Ensure it starts with a drive letter or slash properly, but sqlite:/// + C:/ works.
            return path_str
            
        # Transit DB — use Supabase (PostgreSQL) if DATABASE_URL is set, else SQLite fallback
        if db_type == "transit":
            transit_url = cls._get_env("DATABASE_URL", "")
            if transit_url and "postgresql" in transit_url:
                if is_async:
                    transit_url = transit_url.replace("postgresql://", "postgresql+asyncpg://", 1)
                    if "?" in transit_url:
                        transit_url = transit_url.split("?")[0]
                elif "sslmode" not in transit_url:
                    separator = "&" if "?" in transit_url else "?"
                    transit_url = f"{transit_url}{separator}sslmode=require"
                return transit_url
            # Fallback to local SQLite (dev without Supabase)
