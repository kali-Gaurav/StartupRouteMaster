...
from database.infrastructure.base import AuditMixin,TimestampMixin, Base, UserBase, TransitBase
...
# from database.infrastructure.base import Base, UserBase, TransitBase  <- This was changed to import Base directly
# from database.infrastructure.base import UserBase, TransitBase # This was removed to avoid circularity
...
from core.data_utils.structures import QuotaType, BookingStatus, EscrowStatus, Persona, AllocationStatus, Route
...
class Booking(UserBase):
    ...
    journey_id: Mapped[Optional[str]] = mapped_column(String(36), index=True, nullable=True)
    class_type: Mapped[Optional[str]] = mapped_column(String(10), nullable=True)
    from_station_code: Mapped[Optional[str]] = mapped_column(String(10), nullable=True)
    to_station_code: Mapped[Optional[str]] = mapped_column(String(10), nullable=True)
...
class SeatInventory(UserBase):
    ...
    __table_args__ = (
        CheckConstraint('available_seats >= 0', name='chk_seat_inventory_available_seats'),
...
class Subscription(UserBase):
    ...
    features: Mapped[Optional[Dict[str, Any]]] = mapped_column(JSON, nullable=True, default=dict)
...
Station = Stop
RouteModel = TrainRoute
Train = TrainMaster
# Route is a dataclass in core.data_utils.structures, we keep it as is
...
class TelegramUser(UserBase):
    ...
    # Linked system user
    linked_user_id: Mapped[Optional[str]] = mapped_column(String(36), ForeignKey("users.id"), nullable=True, index=True)
    
    # Preferences
    notifications_enabled: Mapped[bool] = mapped_column(Boolean, default=True)
    sos_enabled: Mapped[bool] = mapped_column(Boolean, default=True)
    ...
    # Relationships
    sessions: Mapped[List["TelegramSession"]] = relationship("TelegramSession", back_populates="telegram_user", cascade="all, delete-orphan")
    messages: Mapped[List["TelegramMessage"]] = relationship("TelegramMessage", back_populates="telegram_user", cascade="all, delete-orphan")
    bookings: Mapped[List["TelegramBookingLink"]] = relationship("TelegramBookingLink", back_populates="telegram_user", cascade="all, delete-orphan")
    sos_events: Mapped[List["TelegramSOSEvent"]] = relationship("TelegramSOSEvent", back_populates="telegram_user", cascade="all, delete-orphan")
    
    def __repr__(self):
        return f"<TelegramUser(telegram_id={self.telegram_id}, name={self.first_name})>"
...
class TelegramSession(UserBase):
    ...
    # Context data (JSON)
    context_data: Mapped[Dict[str, Any]] = mapped_column(JSON, default={}, nullable=False)
    ...
    # Relationships
    telegram_user: Mapped["TelegramUser"] = relationship("TelegramUser", back_populates="sessions")
    
    def __repr__(self):
        return f"<TelegramSession(user={self.telegram_user_id}, state={self.current_state})>"
