...
from backend.core.metrics import CacheMetricsCollector
from backend.core.utils import CacheKeyGenerator

logger = logging.getLogger(__name__)


# Dataclasses now imported from shared modules:
# - RouteQuery
# - AvailabilityQuery
# - CacheMetrics (replaced by CacheMetricsCollector)
...
class MultiLayerCache:
    ...
    def __init__(self):
        ...
        # ✨ NEW: Use unified metrics collectors from shared infrastructure
        self.metrics = {
            'query_cache': CacheMetricsCollector(name='query_cache'),
            'partial_route_cache': CacheMetricsCollector(name='partial_route_cache'),
            'availability_cache': CacheMetricsCollector(name='availability_cache'),
            'ml_cache': CacheMetricsCollector(name='ml_cache')
        }
