...
* Dynamic Hub Selection (Top 250 stops).
* **Neural Pruning (AI-RAPTOR)**: Use a lightweight model to prune RAPTOR rounds by predicting sub-optimal path trajectories.
* **Multi-Modal Transfer Prediction**: Predict actual transfer times at major hubs (NDLS, HWH) using historical platform/delay data.
* **Pareto-Frontier Search**: Optimization for {Time, Cost, Comfort, Reliability} simultaneously, not just shortest path.
* **Bit-Parallel RAPTOR**: Accelerated graph traversal using bitmask operations for regional clusters.
* **Contraction Hierarchies (CH)**: Pre-compute "shortcuts" for the walking transfer graph to achieve sub-millisecond last-mile search.
* **Transfer Patterns**: Pre-calculate optimal interchange points (Bast et al.) to reduce RAPTOR state space by 70%.
...
- "Wait-Time Virtualization" for station displays, showing the _predicted_ wait-time based on ML even if official data is missing.
...
- "Identity-Abstraction" for fare payments, ensuring the transit provider knows a ticket is valid without knowing _who_ is travelling.
