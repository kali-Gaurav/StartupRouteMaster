...
export const INTENTS: ChatIntent[] = [
  ...
  {
    keywords: ["search trains", "find trains"],
...
export function processLocalIntent(text: string): ProcessedIntent | null {
  ...
  const formatDateToISO = (): string | undefined => {
    ...
    try {
      ...
      // Fallback: try parsing the formatted text
    }
  ...
  const dateInfo = formatDateToISO();
  ...
  if (routeMatch && routeMatch[1] && routeMatch[2] && routeMatch[1].length > 2 && routeMatch[2].length > 2) {
    ...
    if (!ignoreList.includes(src) && !ignoreList.includes(dest)) {
      const dateDisplay = dateInfo ? new Date(dateInfo).toLocaleDateString('en-IN', { year: 'numeric', month: 'short', day: 'numeric' }) : '';
      return {
        reply: `🔍 Found your search: **${src}** → **${dest}**${dateDisplay ? (' on ' + dateDisplay) : ''}. Filling the form and searching available trains...`,
        isLocal: true,
        triggerSearch: true,
        collected: {
          source: src,
  ...
  if (places.length >= 2) {
    const dateDisplay = dateInfo ? new Date(dateInfo).toLocaleDateString('en-IN', { year: 'numeric', month: 'short', day: 'numeric' }) : '';
    return {
      reply: `🔍 Found your search: **${places[0]}** → **${places[1]}**${dateDisplay ? (' on ' + dateDisplay) : ''}. Filling the form and searching available trains...`,
      isLocal: true,
      triggerSearch: true,
      collected: {
